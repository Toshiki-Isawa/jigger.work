class Public::CocktailsController < ApplicationController
  ...
  def index
    @cocktails = Cocktail.all.page(params[:page]).without_count.per(8)
  ...
  def search
    if params[:search_key] == "base"
      @cocktails = Cocktail.where(base_name: params[:name]).page(params[:page]).without_count.per(50)
      @page_title = "#{params[:name]}ベースの"
    elsif params[:search_key] == "alcohol"
      if params[:name] == "strong"
        @cocktails = Cocktail.where(alcohol: 25..Float::INFINITY).page(params[:page]).without_count.per(50)
        @page_title = "アルコール度数25%以上の"
      elsif params[:name] == "normal"
        @cocktails = Cocktail.where(alcohol: 10..25).page(params[:page]).without_count.per(50)
        @page_title = "アルコール度数10%〜25%の"
      elsif params[:name] == "weak"
        @cocktails = Cocktail.where(alcohol: -Float::INFINITY..10).page(params[:page]).without_count.per(50)
        @page_title = "アルコール度数10%〜25%の"
      elsif params[:name] == "none"
        @cocktails = Cocktail.where(alcohol: 0).page(params[:page]).without_count.per(50)
        @page_title = "ノンアルコール"
      end
    elsif params[:search_key] == "technique"
      @cocktails = Cocktail.where(technique_name: params[:name]).page(params[:page]).without_count.per(50)
      @page_title = "#{params[:name]}"
    elsif params[:search_key] == "taste"
      @cocktails = Cocktail.where(taste_name: params[:name]).page(params[:page]).without_count.per(50)
      @page_title = "#{params[:name]}な味わいの"
    elsif params[:search_key] == "style"
      @cocktails = Cocktail.where(style_name: params[:name]).page(params[:page]).without_count.per(50)
      @page_title = "#{params[:name]}スタイル"
    elsif params[:search_key] == "tpo"
      @cocktails = Cocktail.where(tpo_name: params[:name]).page(params[:page]).without_count.per(50)
      @page_title = "#{params[:name]}の"
      
    # 材料検索
    elsif params[:search_key] == "ingredient" 
      search_ingredient = Ingredient.find_by(name: params[:name])
      @cocktails = Cocktail.includes(:ingredient_relations).where(ingredient_relations: {ingredient_id: search_ingredient.id}).page(params[:page]).without_count.per(50)
  ...
  def multiple_search
    ...
    @cocktails = Cocktail.search(@search_params).page(params[:page]).without_count.per(50)
    @new_cocktails = Cocktail.order(created_at: 'DESC').limit(14)
    render 'public/cocktails/index'
  end
  ...
  def show
    ...
    @taste_cocktails = Cocktail.where(taste_name: @cocktail.taste_name).distinct.where.not(id: @cocktail.id).shuffle.take(3)
    @rate = Rate.new
  end
